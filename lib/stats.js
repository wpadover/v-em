...
function addStats(players, teamName, stats, $) {
  ...
  findStatsForTeam($, teamName).each(function(index, element) {

    // Get the player's name from our format (FIRST LAST)
    var number = $(element).children('td').eq(NUMBER_INDEX).text().trim();
    var position = $(element).children('td').eq(POSITION_INDEX).text().trim();
    ...
    // in our above array
    $(element).children('td').slice(STATS_INDEX).each(function(index, element) {

      //TODO - better handling of numbers
      playerStats[name][STAT_NAMES[index]] = $(element).text().trim() || "0";
